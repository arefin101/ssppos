...
const AddPaymentMethod = () => {
    ...
    const validation = useFormik({
        ...
        validationSchema: Yup.object({
            name: Yup.string().required("Please Enter Payment Method"),
        }),

        onSubmit: (values) => {
            document.getElementById("submit").disabled = true;
            ref.current.continuousStart();
            dispatch(createPaymentMethod(values, history));
        },
    });
    ...
    const errornotify = () => toast("Payment Method Creation Failed !", { position: "top-right", hideProgressBar: true, closeOnClick: false, className: 'bg-danger text-white' });
    ...
    return (
        <React.Fragment>
            ...
            <LoadingBar color='rgba(240, 101, 72, 0.85)' ref={ref} />
                ...
                <Container fluid>
                    <BreadCrumb title="Add Payment Method" pageTitle="Payment Methods" link="/payment-methods"/>
                    <Row>
                        <Col lg={12}>
                            <Card>
                                <CardHeader className="d-flex">
                                    <h4 className="card-title mb-0 flex-grow-1">Add Payment Method</h4>
                                </CardHeader>
                                    ...
                                    <div className="live-preview">
                                        <Form className="needs-validation" onSubmit={(e) => validate(e)}>
                                            <Row>
                                                <Col md={6}>
                                                    <div className="mb-3">
                                                        <Label htmlFor="Nameinput" className="form-label">Name<span className='text-danger' style={{ fontSize:"10px", verticalAlign: "top" }}><i className="mdi mdi-asterisk"></i></span></Label>
                                                        <Input
                                                            name="name"
                                                            placeholder="Enter new payment method"
                                                            type="text"
                                                            className="form-control"
                                                            id="validation_payment_method_name"
