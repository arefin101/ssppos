      ...
      <div className={divClass}>
        <Table hover {...getTableProps()} className={tableClass}>
...

          {
            ...
            <tfoot>
              <tr>
                  <td className="text-center" colSpan="13">No Data Found</td>
              </tr>
            </tfoot> :
            <tfoot>
              <tr>
                  <td className="text-center fs-16" colSpan="6"><strong>Total:</strong></td>
                  <td>
                  ...
                  <td>
                      <span className="text-dark fs-16 ">
                        ${page.map(data => data.original).reduce((total, row) =>  total = total + parseFloat(row.amount_credited), 0 ).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
                      </span>
                  </td>
                  <td>
                      <span className="text-dark fs-16 ">
                        ${page.map(data => data.original).reduce((total, row) =>  total = total + parseFloat(row.amount_adjusted), 0 ).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
                      </span>
                  </td>
                  <td colSpan="4"></td>
              </tr>
