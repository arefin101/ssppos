    ...
    <Fragment>
      ...
      <div className={divClass}>
        <Table hover {...getTableProps()} className={tableClass}>
          ...
          <tfoot>
            <tr>
                <td className="text-center fs-16" colSpan="3"><strong>Total:</strong></td>
                <td>
                    <span className="text-dark fs-16 ">${page.map(data => data.original).reduce((total, row) =>  total = total + parseFloat(row.gross_profit), 0 ).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</span>
                </td>
                <td colSpan="2"></td>
                <td>
                    <span className="text-dark fs-16 ">${page.map(data => data.original).reduce((total, row) =>  total = total + parseFloat(row.risk_fund), 0 ).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</span>
                </td>
            </tr>
          </tfoot>}
