...
import { withdrawCustomerCredit, getAddCustomerCreditView, getAvailableCustomerCredit } from '../../../store/actions';
import LoadingBar from 'react-top-loading-bar';



const WithdrawCustomerCredit = (props) => {
    ...
    const validation = useFormik({
        ...
        onSubmit: (values, {resetForm}) => {
            ...
            dispatch(withdrawCustomerCredit(id, alteredValues, props.history));
    ...
    useEffect(() => {
        if(error){
            if( (error.amount) || (error.note) ){
                ...
                errornotify("Customer Credit Withdraw Failed !");
            }
        }
    }, [error]);
    ...
    if(localStorage.getItem('withdrawCustomerCredit')) {
        successnotify("Amount Withdrawn Successfully !");
        localStorage.removeItem('withdrawCustomerCredit');
        document.getElementById("submit").disabled = false;
        ref.current.complete();
    }
    ...
    return (
        <React.Fragment>
            ...
            <div className="page-content">
                ...
                <MetaTags>
                    <title>Withdraw Customer Credit</title>
                </MetaTags>
                <Container fluid>
                    <BreadCrumb title="Withdraw Customer Credit" pageTitle="Money Transactions" link="/money-transactions"/>
                    <Row>
                        <Col lg={12}>
                        <Card>
                            <PreviewCardHeader title="Withdraw Customer Credit" />
                            <CardBody>
                                        ...
                                        <Row>
                                            <Col md="6">
                                                <Row>
                                                    <Col md="12">
                                                        <div className="mb-3">
                                                            <Label htmlFor="validationCustomerName">Customer<span className='text-danger' style={{ fontSize:"10px", verticalAlign: "top" }}><i className="mdi mdi-asterisk"></i></span></Label>                                                   
                                                            <Select
                                                                isClearable={true}
...
export default WithdrawCustomerCredit
