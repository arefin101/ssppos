    ...
    <Fragment>
      ...
      <div className={divClass}>
        <Table hover {...getTableProps()} className={tableClass}>
          ...
          <tfoot>
            <tr>
                <td className="text-center fs-16" colSpan="0"><strong>Total:</strong></td>
                <td>
                    <span className="text-dark fs-16 ">${page.map(data => data.original).reduce((total, row) =>  total = total + parseFloat(row.commission), 0 ).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</span>
                </td>
                <td>
                    <span className="text-dark fs-16 ">${page.map(data => data.original).reduce((total, row) =>  total = total + parseFloat(row.commission_return), 0 ).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</span>
                </td>
                <td>
                    <span className="text-dark fs-16 ">${page.map(data => data.original).reduce((total, row) =>  total = total + (parseFloat(row.commission) - parseFloat(row.commission_return)), 0 ).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</span>
                </td>
            </tr>
          </tfoot>}
        </Table>
