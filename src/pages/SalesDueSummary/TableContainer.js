        ...
        <Table hover {...getTableProps()} className={tableClass}>
          ...
          <tfoot>
            <tr>
                ...
                <td>
                    <span className="text-dark fs-16 ">${page.map(data => data.original).reduce((total, row) =>  total = total + parseFloat(row.due), 0 ).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</span>
                </td>
            </tr>
          </tfoot>}
        </Table>
