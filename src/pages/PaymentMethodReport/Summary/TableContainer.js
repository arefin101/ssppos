...
const TableContainer = ({
  ...
  const from = filteredLength - (entriesCalculation - ((pageSize*pageOptions.length)-filteredLength)) + 1;
              ...
              <tr>
                  ...
                  <td>
                      <span className="text-dark fs-16 ">
                        ${page.map(data => data.original).reduce((total, row) =>  total = total + parseFloat(row.sale_transaction), 0 ).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
                      </span>
                  </td>
                  <td>
                      <span className="text-dark fs-16 ">
                        ${page.map(data => data.original).reduce((total, row) =>  total = total + parseFloat(row.purchase_transaction), 0 ).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
                      </span>
                  </td>
                  <td>
                      <span className="text-dark fs-16 ">
                        ${page.map(data => data.original).reduce((total, row) =>  total = total + parseFloat(row.expense_transaction), 0 ).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
                      </span>
                  </td>
                  <td>
                      <span className="text-dark fs-16 ">
                        ${page.map(data => data.original).reduce((total, row) =>  total = total + parseFloat(row.total_amount), 0 ).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
                      </span>
                  </td>
              </tr>
            </tfoot>
