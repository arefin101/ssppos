...
const TableContainer = ({ columns, data, isGlobalSearch, customPageSize, tableClass, theadClass, thClass, divClass, setFilteredData, isExtraFeature, maxLength, filteredLength, transactionCount, totalAmount }) => {
  const {
    getTableProps,
    getTableBodyProps,
    headerGroups,
    ...
    <Fragment>
        {isExtraFeature && (
          <Row className="mb-2">
            ...
            <Col md={4} xxl={7} sm={2}></Col>
            <Col md={6} xxl={4} sm={6} className="mt-3 text-end">
                {transactionCount && totalAmount && <div className="hstack gap-3 flex-wrap justify-content-end">
                  <div className="fs-16"> Transaction Count :{" "} <span className="fw-medium text-primary"> {transactionCount} </span></div> 
                  <div className="vr"></div> 
                  <div className="fs-16"> Total Amount :{" "} <span className="fw-medium text-primary"> ${totalAmount} </span></div>
                </div> }
            </Col>
      ...
      <div className={divClass}>
        <Table hover {...getTableProps()} className={tableClass}>
...

          {
            ...
            <tfoot>
              <tr>
                  <td className="text-center fs-16" colSpan="2"><strong>Total:</strong></td>
                  <td>
                      <span className="text-dark fs-16 ">
                        ${page.map(data => data.original).reduce((total, row) =>  total = total + parseFloat(row.amount), 0 ).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
                      </span>
                  </td>
                  <td></td>
              </tr>
