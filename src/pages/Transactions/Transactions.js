...
import { getTransactionRecords } from "../../store/actions";
...
const Transactions = () => {
    ...
    useEffect(()=>{
        dispatch( getTransactionRecords(history) );
    }, []);

    const { response } = useSelector(state => ({
        response: state.Record.transactions,
    }));
    ...
    const searchItems = (searchValue) => {
        ...
        if (searchValue !== '') {
            ...
            const filteredData = response.transactions.filter((item) => {
                                    ...
                                    <Col sm={12} xxl={4}>
                                        <div className="search-box">
                                            <input
                                                ...
                                                placeholder="Search for type, amount or something..."
                                ...
                                {   
                                    ...
                                    <div>
                                        <TableContainer
                                            ...
                                            data={transactions}
                                            customPageSize={10}
                                            maxLength={response.transactions ? response.transactions.length : 10}
                                            filteredLength={transactions.length} 
                                            isExtraFeature={true}
                                            isGlobalSearch={true}
